...
namespace JetBrains.FormatRipper.Tests
{
  ...
  public sealed class CompoundFileTest
  {
    public readonly struct StreamInfo
    {
      ...
      public StreamInfo(string hash, params string[] names) :
        this(Guid.Empty, hash, names)
      {
      }

      public StreamInfo(Guid clsid, string hash, params string[] names)
      {
        Names = names;
        Clsid = clsid;
        Hash = hash;
    ...
    private static object?[] MakeSource(string filename, CompoundFile.FileType type, CodeOptions options, string? cmsSignatureHash, string includeRanges, params StreamInfo[] streams)
    {
      return new object?[] { filename, type, options, cmsSignatureHash, includeRanges, streams };
    }

    private static object?[] Sources =
      {
        MakeSource("2dac4b.msi", CompoundFile.FileType.Msi, CodeOptions.HasCmsSignature, "4D628DAE27082C2FC80D0FD9AAEC6FD095DFCAE306F671A987E785BE13F6F684727F118B71143687D5A440352786218D", "0;0;[4000:278],[4AC0:12],[4B40:C],[4B00:20],[4B80:3C],[4C80:E],[4D00:60],[4E00:248],[6000:1CC5],[5080:3AC],[4BC0:78],[4C40:24],[4CC0:3C],[4DC0:1E],[4A80:24],[49C0:30],[4A00:18],[4A40:2A],[4D80:10],[4280:708],[2050:10]",
    ...
    [Test]
    public void Test(string filename, CompoundFile.FileType expectedType, CodeOptions expectedOptions, string? expectedCmsSignatureBlobHash, string expectedIncludeRanges, StreamInfo[] expectedStreams)
    {
      var file = ResourceUtil.OpenRead("Msi." + filename, stream =>
        {
          Assert.IsTrue(CompoundFile.Is(stream));
          return CompoundFile.Parse(stream, CompoundFile.Mode.ReadCodeSignature, (_, _, _) => true);
        });
